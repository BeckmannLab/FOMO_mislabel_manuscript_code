...
filtered_bam_file="${temp_dir}/${sample_id}.filter.bam"
sorted_bam_file="${temp_dir}/${sample_id}.filter.sort.bam"
...
java -jar $PICARD ReorderSam I="${input_bam_file}" O="${filtered_bam_file}" SD="${reference_dict}" S=true TMP_DIR="${TMP_DIR}" MAX_RECORDS_IN_RAM=200000
echo "3. Sorting filtered BAM file for sample ${sample_id}"
samtools sort ${filtered_bam_file} -o "${sorted_bam_file}"
...
java -jar $PICARD ExtractFingerprint I="${sorted_bam_file}" R="${reference_file}" H="${map_file}" O="${output_file}"
echo "6. Done extracting fingerprint for ${sample_id}"
