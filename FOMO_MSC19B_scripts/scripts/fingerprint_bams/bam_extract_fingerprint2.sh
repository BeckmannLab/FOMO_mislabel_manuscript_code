...
export TMPDIR="[PROJECT_DIR]/MSCBB/temp_dir"
export TMP="$TMPDIR"
export TEMP="$TMPDIR"
...
java -Xmx8g -jar $PICARD ReorderSam I="${copied_bam_file}" O="${filtered_bam_file}" SD="${reference_dict}" S=true TMP_DIR="$TMP" MAX_RECORDS_IN_RAM=2000000
...
samtools sort "${filtered_bam_file}" -o "${sorted_bam_file}" -m 3G --write-index
