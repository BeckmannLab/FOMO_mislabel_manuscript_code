...
batch_files_vec <- list.files("[PROJECT_DIR]/MSCBB/crosscheck_batches", pattern=".csv$", full.names=TRUE)
...
command_constructor_df <- data.frame(batch_file=batch_files_vec) %>%
    mutate(
        ...
        command = glue("Rscript [PROJECT_DIR]/MSCBB/scripts/make_genotype_matrix/run_batch_crosscheck_comparisons.R {batch_file}"),
